...
static void globalDeclarator(int typespec)
{
	...
	match('[');
	global_vars.push_back(declareVariable(name, Type(typespec, indirection, number())));
	match(']');

    } else
	global_vars.push_back(declareVariable(name, Type(typespec, indirection)));
}
