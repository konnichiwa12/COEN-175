...
bool Type::isInteger() const { // only scalars that aren't pointers or void
...
Type Type::promote() const {
    if (_kind == SCALAR && _specifier == CHAR && _indirection == 0) { // char to int
        return Type(INT);
    }
    if (_kind == ARRAY) { // array to pointer
...
bool Type::isCompatibleWith(const Type &that) const {
    /* Type this_type = promote();
    Type that_type = that.promote();
    if (this_type.isInteger() && that_type.isInteger()) { // same value types is true
        return true;
    }
    else if (this_type.isPointer() && that_type.isPointer()) { // if both are pointers that have the same types or one is void, then it's true
        if ((this_type.specifier() == VOID || that_type.specifier() == VOID) || (this_type.specifier() == that_type.specifier())) {
            return true;
        }
    }
    return false;
